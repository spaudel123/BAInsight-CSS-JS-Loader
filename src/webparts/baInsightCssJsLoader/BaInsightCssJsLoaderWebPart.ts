        ...
        <div class="${ styles.container }">
          <div class="${ styles.row }">
            <div class="${ styles.column }">
              <span class="${ styles.title }">Load External CSS/JS + Displays spinning wheel</span>
  ...
  protected onInit<T>():Promise<T>{
    ...
    return new Promise<T>((resolve: (args: T) => void, reject: (error: Error) => void) => {
      ...
      setTimeout(() => {
        ...
        //unblur
        let allbutspinner = document.querySelectorAll("body > *:not(#theSpinner)");
        for(var i=0;i<allbutspinner.length;i++)
        {
          (allbutspinner[i] as HTMLElement).style.filter = "";
        }
        //remove the spinner
        let spinner = document.getElementById("theSpinner");
        if(spinner)
          document.getElementById("theSpinner").remove();
        resolve(undefined);
        }, 4000);
    });
    //return super.onInit();
